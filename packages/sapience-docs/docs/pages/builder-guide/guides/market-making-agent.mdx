...
2. Listen for `auction.started { auctionId, taker, wager, predictedOutcomes[], resolver, chainId, marketContract }`
...
- Market metadata: decodes `auction.predictedOutcomes[0]` and fetches condition titles via GraphQL for readable logs
- Signing: uses helpers to build and sign the EIP‑712 maker bid payload with domain `{ chainId: auction.chainId, verifyingContract: auction.marketContract }` and inner hash `{ encodedPredictedOutcomes, makerWager, wager, resolver, taker, makerDeadline }`
...
const { domain, types, primaryType, message } = buildMakerBidTypedData({
  ...
  verifyingContract: auction.marketContract,
  maker: MAKER,
});
...
- Filter by topic or condition group by inspecting decoded `predictedOutcome`
