...
- **Generates Forecasts**: Analyzes parlay conditions and publishes forecasts to [Ethereum Attestation Service](https://docs.attest.sh/)
...
- ✅ **Parlay Trading**: Enabled by default - places multi-leg wagers based on high-confidence predictions
...
| `ENABLE_PARLAY_TRADING` | `false` | Enable automated parlay trading based on forecasts (requires explicit enabling) |
...
3. **Operational Behavior**:
   ...
   - **De-duplication**: Won't re-attest the same market within 24 hours unless probability changes >10%
...
**Execution Frequency:**
- Parlay opportunities are checked every attestation cycle (default: every 5 minutes)
- Maximum one parlay attempt per 24 hours (prevents overtrading)
- Each auction stays open for 5 minutes waiting for takers

**What to Monitor:**
- Log messages: `[ParlayMarket] Found X active parlay conditions` 
- Successful parlays: Look for transaction hash in logs
- Failed attempts: Bot will retry after the cooldown period

**Code Reference:**
[View the parlay trading implementation →](https://github.com/sapiencexyz/sapience/blob/main/starters/elizaos/src/actions/parlayTradingAction.ts)
