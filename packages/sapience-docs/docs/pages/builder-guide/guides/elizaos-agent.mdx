...
The [ElizaOS starter](https://github.com/sapiencexyz/sapience/tree/main/starters/elizaos) is a complete autonomous forecasting agent built with the [ElizaOS framework](https://elizaos.ai/). It can generate forecasts on prediction markets, publish forecasts onchain, and execute trades through Sapience's auction system.
...
Trigger a forecast cycle that analyzes prediction markets and publishes onchain attestations:
...
4. Submit forecasts as onchain attestations (if a private key is configured)

### Trade

Execute trades on prediction markets based on the agent's analysis:
...
5. Execute the trade onchain

**Note:** Trading requires USDe tokens in your wallet on Ethereal.
