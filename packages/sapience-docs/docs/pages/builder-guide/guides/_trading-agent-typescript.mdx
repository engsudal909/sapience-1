...
1. **Fetch a Market** - Gets one active prediction market
...
// 1. Fetch one active market from Sapience API
async function fetchMarket() {
  const query = gql`
    query {
      markets(where: { question_not: null }, orderBy: endTimestamp, first: 1) {
        id
        question
      }
    }
...
async function main() {
  ...
  // 4. Start auction
  const encodedOutcomes = encodePrediction(market.id, forecast.probability);
