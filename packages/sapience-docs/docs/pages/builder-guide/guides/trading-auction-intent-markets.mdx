...
Takers initiate auctions by sending `auction.start`. The relayer broadcasts `auction.started` with parameters including `auctionId`, `taker`, `wager`, `resolver`, and `predictedOutcomes`.
