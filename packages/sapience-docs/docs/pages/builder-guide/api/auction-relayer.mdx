...
1) Taker sends `auction.start` with `taker`, `wager`, `predictedOutcomes[]`, `resolver`, `takerNonce`, `chainId`, `marketContract`.
...
- **Auction**: positive `wager`, ≥1 `predictedOutcomes` (non-empty bytes strings), valid `resolver` address, valid `taker` address and `chainId`, valid `marketContract`.
- **Bid**: `makerDeadline` in the future, positive `makerWager`, valid hex `makerSignature` format
- Optional strict signature verification uses EIP-712 with domain `{ chainId: auction.chainId, verifyingContract: auction.marketContract }` and inner hash over `encodedPredictedOutcomes`, `makerWager`, `wager`, `resolver`, `taker`, `makerDeadline` (see Reference).
