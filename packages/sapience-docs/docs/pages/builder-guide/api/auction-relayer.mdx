...
1) Taker sends `auction.start` with `taker`, `wager`, `resolver`, `predictedOutcomes[]`, `takerNonce`, `chainId`.
...
- Takers are automatically subscribed to their auction's channel upon `auction.start`.
...
- **Auction**: positive `wager`, ≥1 `predictedOutcomes`, valid `resolver`, valid `taker` address
