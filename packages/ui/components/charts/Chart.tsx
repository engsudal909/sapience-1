...
const foilGet = async (path: string) => {
  const base = process.env.NEXT_PUBLIC_FOIL_API_URL || 'https://api.sapience.xyz';
...
import type { TimeInterval, TimeWindow } from '@sapience/sdk/types';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export const Chart = ({
  ...
  const { data: _chartData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return foilGet(`/charts/${resourceSlug}?${params.toString()}`);
    },
