...
contract PredictionMarketPythBenchmarkResolverArbitrumForkTest is Test {
    ...
    function test_e2e_arbitrumFork_settleMarket() public {
        ...
        bytes32 priceId = vm.envOr("PYTH_PRICE_ID", ArbitrumBenchmarkFixture.BTC_USD_PRICE_ID);
        ...
        bytes memory envOverride = vm.envOr("PYTH_BENCHMARK_UPDATE_DATA", bytes(""));
        updateData[0] = envOverride.length > 0
            ? envOverride
            : ArbitrumBenchmarkFixture.BENCHMARK_UPDATE_DATA;
