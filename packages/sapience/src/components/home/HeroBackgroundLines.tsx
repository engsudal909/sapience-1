	...
	useEffect(() => {
		...
		function seedLines() {
			...
			for (let i = 0; i < numLines; i++) {
				...
				const spawnOffset = Math.random() * canvas.clientWidth;
				result.push({
					x: -width - spawnOffset,
		...
		function drawGradientLine(x: number, y: number, width: number, opacity: number) {
			const h = 2; // 2px tall
		...
		function frame(now: number) {
			...
			for (const line of linesRef.current) {
				...
				if (line.x > canvas.clientWidth + 40) {
					const newWidth = 60 + Math.random() * 300;
					const respawnOffset = Math.random() * canvas.clientWidth * 0.75; // stagger re-entry
